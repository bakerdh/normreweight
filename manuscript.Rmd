...
# 1 - generate figures using the processed group data (requires 0.4GB of storage)
# 2 - download processed individual data and analyse (requires 11.5GB of storage)
# 3 - download all raw data and analyse (requires 143GB of storage)

plotsmoothed <- 1 # either plot the raw data (0), or a smoothed spline (1)
fixedphase <- 0 # option to use a fixed phase sinusoidal dot product instead of Fourier analysis for the timecourses
...
packagelist <- c('knitr','remotes','tictoc','R.matlab','bookdown','rmarkdown','grImport','tiff','pals','ez','gtools','pracma','osfr') # list of CRAN packages
...
```{r analysepilotindividual, include=FALSE, results='hide'}
...
tindex <- (10*targetF) + 1
mindex <- (10*maskF) + 1
tindex2 <- (10*targetF*2) + 1
mindex2 <- (10*maskF*2) + 1
duration <- 14
...
```{r analysepilotgroup, include=FALSE, results='hide'}
...
if (fixedphase==0){  # do normal Fourier analysis
...
if (fixedphase==1){  # used fixed phase dot product with sinewave
  # make a sine wave with the same phase as the EEG response
  spec <- fft(temp)/length(temp)
  synspec <- spec*0
  synspec[(targetF*14) + 1] <- spec[(targetF*14) + 1]
  sw1 <- Re(fft(synspec,inverse=TRUE))
  sw1 <- sw1/max(sw1)
  synspec <- spec*0
  synspec[(maskF*14) + 1] <- spec[(maskF*14) + 1]
  sw2 <- Re(fft(synspec,inverse=TRUE))  
  sw2 <- sw2/max(sw2)
  synspec <- spec*0
  synspec[(2*targetF*14) + 1] <- spec[(2*targetF*14) + 1]
  sw3 <- Re(fft(synspec,inverse=TRUE))
  sw3 <- sw3/max(sw3)
  synspec <- spec*0
  synspec[(2*maskF*14) + 1] <- spec[(2*maskF*14) + 1]
  sw4 <- Re(fft(synspec,inverse=TRUE)) 
  sw4 <- sw4/max(sw4)

for (t in 1:(14000-binwidth)){
  EEGtimecourse[s,1,cond,t] <- mean(temp[t:(t+binwidth-1)] * sw1[t:(t+binwidth-1)]) + 0.1
  EEGtimecourse[s,2,cond,t] <- mean(temp[t:(t+binwidth-1)] * sw2[t:(t+binwidth-1)]) + 0.1
  EEGtimecourse[s,3,cond,t] <- mean(temp[t:(t+binwidth-1)] * sw3[t:(t+binwidth-1)]) + 0.1
  EEGtimecourse[s,4,cond,t] <- mean(temp[t:(t+binwidth-1)] * sw4[t:(t+binwidth-1)]) + 0.1
}}    
...
```{r plotpilotdata, include=FALSE, results='hide'}
...
legend(2,1,c('Baseline','Target','Target+mask'),col=c('grey',colpal[1],colpal[2]),lty=1,lwd=2,box.lwd=2)
...
spl5aL <- smooth.spline(times,EEGmeantimes[2,2,]-EEGSEtimes[2,2,],df=20)
 # use of pmin function here crops the error region at the top of the graph 
polygon(times[c(1:13000,13000:1)],c(pmin(spl5aU$y,1),spl5aL$y[13000:1]),col=colpal[1],border=NA)
...
dev.off()
 ...
 if (fixedphase==0){pdf('Figures/VilidaiteData.pdf', bg="transparent", height = 15, width = 6)}
 if (fixedphase==1){pdf('Figures/VilidaiteDataFP.pdf', bg="transparent", height = 15, width = 6)}  
  par(mar=c(0.1,0.1,0.1,0.1))
  plot(x=NULL,y=NULL,axes=FALSE, ann=FALSE, xlim=c(0,1), ylim=c(0,1)) 

  aspratio <- 15/6  # this is the aspect ratio of the output pdf
  imwidth <- 0.05
  xstart <- 0.05
  ystart <- 0.92
  ...
  grid.picture(e3,x=0.5,y=0.91,width=1)
  grid.picture(e5,x=0.5,y=0.74,width=1)

  grid.picture(e6,x=0.25,y=0.57,width=0.5,height=1)
  grid.picture(e7,x=0.75,y=0.57,width=0.5,height=1)
  grid.picture(e8,x=0.25,y=0.42,width=0.5,height=1)
  grid.picture(e9,x=0.75,y=0.42,width=0.5,height=1)
  
  grid.picture(e4,x=0.5,y=2/12,width=1)
  
  text(-0.055,1.02,'(a)',pos=4,cex=2)
  text(-0.055,0.85,'(b)',pos=4,cex=2)
  text(-0.055,0.67,'(c)',pos=4,cex=2)
  text(-0.055,0.3,'(d)',pos=4,cex=2)
...
```{r pilotstats, include=FALSE, results='hide'}
...
baseline <- rowMeans((20*log10(abs(EEGtimecourse[,1,1,1001:2000])/abs(EEGtimecourse[,1,3,1001:2000])) + 20*log10(abs(EEGtimecourse[,2,2,1001:2000])/abs(EEGtimecourse[,2,3,1001:2000])) + 20*log10(abs(EEGtimecourse[,3,1,1001:2000])/abs(EEGtimecourse[,3,3,1001:2000])) + 20*log10(abs(EEGtimecourse[,4,2,1001:2000])/abs(EEGtimecourse[,4,3,1001:2000])))/4)

laterepoch <- rowMeans((20*log10(abs(EEGtimecourse[,1,1,3001:4000])/abs(EEGtimecourse[,1,3,3001:4000])) + 20*log10(abs(EEGtimecourse[,2,2,3001:4000])/abs(EEGtimecourse[,2,3,3001:4000])) + 20*log10(abs(EEGtimecourse[,3,1,3001:4000])/abs(EEGtimecourse[,3,3,3001:4000])) + 20*log10(abs(EEGtimecourse[,4,2,3001:4000])/abs(EEGtimecourse[,4,3,3001:4000])))/4)

diff <- laterepoch-baseline

piloteffectsize <- mean(diff)/sd(diff)

```

```{r analyseEEGindividual, include=FALSE, results='hide'}
...
for (datahalf in 1:length(EEGID)){
  ...
  for (f in 1:nrow(EEGfiles)){
    subjcounter <- subjcounter + 1
    subID <- unlist(strsplit(EEGfiles$name[f],'.zip'))
    print(subID)
    # first check if we have this participant's data
    if (!file.exists(paste0('temp/EEGprocessed/',subID,'data.RData'))){
      # if we haven't processed the data, check if we need to download the raw data
      if (!dir.exists(paste0('temp/raw/',subID))){
        if (!file.exists(paste0('temp/raw/',EEGfiles$name[f]))){
          osf_download(EEGfiles[f,], path='temp/raw/',progress=TRUE)
        }
          unzip(paste0('temp/raw/',EEGfiles$name[f]),exdir='temp/raw/')
          file.remove(paste0('temp/raw/',EEGfiles$name[f]))
      }
    # if necessary, process the data for this participant
      
      d <- dir(paste0('temp/raw/',subID), pattern='*.csv.gz',full.names=TRUE)
      condcounter <- (1:4) * 0
    ...
    for (ch in 1:64){     
      ...
      qdatasubj <- read.csv(paste0('temp/raw/',subID,'/questionnairedata.csv'))
      save(file=paste0('temp/EEGprocessed/',subID,'data.RData'),list=c('qdatasubj','condcounter','trialsincluded','subjwaves','electrodes'))
    }
 }
}
...
```{r analyseEEGgroup, include=FALSE, results='hide'}
...
if (fixedphase==1){  # used fixed phase dot product with sinewave
  ...
  spec <- fft(temp)/length(temp)
  synspec <- spec*0
  synspec[(targetF*9) + 1] <- spec[(targetF*9) + 1]
  sw1 <- Re(fft(synspec,inverse=TRUE))
  sw1 <- sw1/max(sw1)
  synspec <- spec*0
  synspec[(maskF*9) + 1] <- spec[(maskF*9) + 1]
  sw2 <- Re(fft(synspec,inverse=TRUE)) 
  sw2 <- sw2/max(sw2)

for (t in 1:(9000-binwidth)){
  EEGtimecourse5Hz[s,cond,t] <- mean(temp[t:(t+binwidth-1)] * sw1[t:(t+binwidth-1)])
  EEGtimecourse7Hz[s,cond,t] <- mean(temp[t:(t+binwidth-1)] * sw2[t:(t+binwidth-1)])
}} 
...
```{r plotEEGdata, include=FALSE, results='hide'}
...
monratios[which(monratios==0)] <- 0.001
EEGmaskindex5[1,] <- colMeans(20*log10(monratios),na.rm=TRUE)
EEGmaskindexSE5[1,] <- apply(20*log10(monratios),2,sd,na.rm=TRUE)/sqrt(sum(includedboth))
...
dichratios[which(dichratios==0)] <- 0.001
EEGmaskindex5[2,] <- colMeans(20*log10(dichratios),na.rm=TRUE)
EEGmaskindexSE5[2,] <- apply(20*log10(dichratios),2,sd,na.rm=TRUE)/sqrt(sum(includedboth))
...
monratios[which(monratios==0)] <- 0.001
EEGmaskindex7[1,] <- colMeans(20*log10(monratios),na.rm=TRUE)
EEGmaskindexSE7[1,] <- apply(20*log10(monratios),2,sd,na.rm=TRUE)/sqrt(sum(includedboth))
...
dichratios[which(dichratios==0)] <- 0.001
EEGmaskindex7[2,] <- colMeans(20*log10(dichratios),na.rm=TRUE)
EEGmaskindexSE7[2,] <- apply(20*log10(dichratios),2,sd,na.rm=TRUE)/sqrt(sum(includedboth))
...
EEGmaskindex5[which(is.na(EEGmaskindex5))] <- 0
EEGmaskindex7[which(is.na(EEGmaskindex7))] <- 0
EEGmaskindexSE5[which(is.na(EEGmaskindexSE5))] <- 0
EEGmaskindexSE7[which(is.na(EEGmaskindexSE7))] <- 0
...
if (plotsmoothed==0){
polygon(times[c(1:8000,8000:1)],pmin(c(EEGmeantime5Hz[1,]+EEGSEtime5Hz[1,],EEGmeantime5Hz[1,8000:1]-EEGSEtime5Hz[1,8000:1]),plotlims[4]),col=colpal[1],border=NA)
...
lines(times,pmin(EEGmeantime5Hz[1,],plotlims[4]),lwd=2,col=colpal[1])
...
polygon(times[c(1:8000,8000:1)],pmin(c(spl5aU$y,spl5aL$y[8000:1]),plotlims[4]), col=colpal[1],border=NA)
...
spl5a <- smooth.spline(times,pmin(EEGmeantime5Hz[1,],plotlims[4]),df=20)
lines(spl5a$x,spl5a$y,col=colpal[1],lwd=2)
...
text(6,0.55,'5Hz',cex=2)
  legend(1.25,0.2,c('Target only','Monocular','Dichoptic'),lty=1,lwd=2,col=colpal[1:3],box.lwd=2)
dev.off()
...
if (fixedphase==0){pdf('Figures/EEGdata.pdf', bg="transparent", height = 6, width = 15)}
if (fixedphase==1){pdf('Figures/EEGdataFP.pdf', bg="transparent", height = 6, width = 15)}
  par(mar=c(0.1,0.1,0.1,0.1))
  plot(x=NULL,y=NULL,axes=FALSE, ann=FALSE, xlim=c(0,1), ylim=c(0,1)) 
...
baseline <- rowMeans((20*log10(abs(EEGtimecourse5Hz[,1,1001:2000])/abs(EEGtimecourse5Hz[,3,1001:2000])) + 20*log10(abs(EEGtimecourse5Hz[,1,1001:2000])/abs(EEGtimecourse5Hz[,4,1001:2000])) + 20*log10(abs(EEGtimecourse7Hz[,2,1001:2000])/abs(EEGtimecourse7Hz[,3,1001:2000])) + 20*log10(abs(EEGtimecourse7Hz[,2,1001:2000])/abs(EEGtimecourse7Hz[,4,1001:2000])))/4)

laterepoch <- rowMeans((20*log10(abs(EEGtimecourse5Hz[,1,3001:4000])/abs(EEGtimecourse5Hz[,3,3001:4000])) + 20*log10(abs(EEGtimecourse5Hz[,1,3001:4000])/abs(EEGtimecourse5Hz[,4,3001:4000])) + 20*log10(abs(EEGtimecourse7Hz[,2,3001:4000])/abs(EEGtimecourse7Hz[,3,3001:4000])) + 20*log10(abs(EEGtimecourse7Hz[,2,3001:4000])/abs(EEGtimecourse7Hz[,4,3001:4000])))/4)

diff <- laterepoch-baseline

EEGeffectsize <- mean(diff)/sd(diff)

```

```{r analyseMEGgroup, include=FALSE, results='hide'}
...
if (fixedphase==1){  # used fixed phase dot product with sinewave
  ...
  spec <- fft(temp)/length(temp)
  synspec <- spec*0
  synspec[(targetF*8) + 1] <- spec[(targetF*8) + 1]
  sw1 <- Re(fft(synspec,inverse=TRUE))
  sw1 <- sw1/max(sw1)
  synspec <- spec*0
  synspec[(maskF*8) + 1] <- spec[(maskF*8) + 1]
  sw2 <- Re(fft(synspec,inverse=TRUE))
  sw2 <- sw2/max(sw2)
  
for (t in 1:(9000-binwidth)){
  MEGtimecourse5Hz[s,cond,t] <- mean(temp[t:(t+binwidth-1)] * sw1[t:(t+binwidth-1)])
  MEGtimecourse7Hz[s,cond,t] <- mean(temp[t:(t+binwidth-1)] * sw2[t:(t+binwidth-1)])
}} 
...
```{r plotMEGdata, include=FALSE, results='hide'}
...
if (fixedphase==0){
  ...
  MEGmaskindexSE7 <- array(0, dim=c(2,8008))
    ...
    MEGmaskindex5[1,] <- colMeans(20*log10(monratios),na.rm=TRUE)
    MEGmaskindexSE5[1,] <- apply(20*log10(monratios),2,sd,na.rm=TRUE)/sqrt(sum(includedboth))
    includedboth <- isincluded[1,]*isincluded[4,]
    dichratios <- abs(MEGtimecourse5Hz[which(includedboth==1),1,])/abs(MEGtimecourse5Hz[which(includedboth==1),4,])
    MEGmaskindex5[2,] <- colMeans(20*log10(dichratios),na.rm=TRUE)
    MEGmaskindexSE5[2,] <- apply(20*log10(dichratios),2,sd,na.rm=TRUE)/sqrt(sum(includedboth))
...
MEGmaskindex5[which(is.na(MEGmaskindex5))] <- 0
MEGmaskindex7[which(is.na(MEGmaskindex7))] <- 0
MEGmaskindexSE5[which(is.na(MEGmaskindexSE5))] <- 0
MEGmaskindexSE7[which(is.na(MEGmaskindexSE7))] <- 0
  ...
  legend(1.25,1,c('Target only', 'Monocular','Dichoptic'), lty=1,lwd=2,col=colpal[1:3],box.lwd=2)
  dev.off()
...
if (fixedphase==0){pdf('Figures/MEGdata.pdf', bg="transparent", height = 6, width = 15)}
if (fixedphase==1){pdf('Figures/MEGdataFP.pdf', bg="transparent", height = 6, width = 15)}
  par(mar=c(0.1,0.1,0.1,0.1))
  plot(x=NULL,y=NULL,axes=FALSE, ann=FALSE, xlim=c(0,1), ylim=c(0,1)) 
...
baseline <- rowMeans((20*log10(abs(MEGtimecourse5Hz[,1,1002:2003])/abs(MEGtimecourse5Hz[,3,1002:2003])) + 20*log10(abs(MEGtimecourse5Hz[,1,1002:2003])/abs(MEGtimecourse5Hz[,4,1002:2003])) + 20*log10(abs(MEGtimecourse7Hz[,2,1002:2003])/abs(MEGtimecourse7Hz[,3,1002:2003])) + 20*log10(abs(MEGtimecourse7Hz[,2,1002:2003])/abs(MEGtimecourse7Hz[,4,1002:2003])))/4)

laterepoch <- rowMeans((20*log10(abs(MEGtimecourse5Hz[,1,3004:4005])/abs(MEGtimecourse5Hz[,3,3004:4005])) + 20*log10(abs(MEGtimecourse5Hz[,1,3004:4005])/abs(MEGtimecourse5Hz[,4,3004:4005])) + 20*log10(abs(MEGtimecourse7Hz[,2,3004:4005])/abs(MEGtimecourse7Hz[,3,3004:4005])) + 20*log10(abs(MEGtimecourse7Hz[,2,3004:4005])/abs(MEGtimecourse7Hz[,4,3004:4005])))/4)

diff <- laterepoch-baseline

MEGeffectsize <- mean(diff)/sd(diff)

```

```{r runIManalysis, include=FALSE, results='hide'}
...
targetF <- 7
maskF <- 5
tindex <- (10*(targetF+maskF)) + 1
duration <- 14

binwidth <- 1000
findex <- 1 + (targetF+maskF)*(binwidth/samplerate)
...
EEGtimecourse <- array(0,dim=c(length(d),3,13000))
# EEGspectra <- array(0,dim=c(length(d),3,20*10))
EEGscalpresps <- array(0,dim=c(length(d),14,64))
# EEGallconds <- array(0,dim=c(length(d),4,14))
alltrialcounts <- array(0,dim=c(length(d),14,64))
...
for (t in 1:(14000-binwidth)){
  spec <- fft(temp[t:(t+binwidth-1)])/binwidth
  EEGtimecourse[s,cond,t] <- spec[findex]
}
} 
...
for (cond in 1:14){
  for (ch in 1:64){
    ...
    temp[nainds] <- 0   # replace with zeros
   spec <- fft(temp[2501:12500])/10000
   EEGscalpresps[s,cond,ch] <- spec[((targetF+maskF)*10)+1]
...
save(file='temp/PilotIM.RData',list=c('EEGtimecourse','electrodes','EEGscalpresps','alltrialcounts'))

}

if (processdata>0){
  
   if (!file.exists('temp/PilotIM.RData')){
      hid <- pmatch('PilotIM.RData',as.character(unlist(osffiles[,1])))
      osf_download(osffiles[hid,],'temp/',progress=TRUE)
 }
load('temp/PilotIM.RData')
  
nsubjs <- dim(EEGtimecourse)[1]
EEGmeantimes <- array(0,dim=c(3,13000))
EEGSEtimes <- array(0,dim=c(3,13000))
EEGmeanscalp <- abs(apply(abs(EEGscalpresps),2:3,mean))

for (cond in 1:3){
EEGmeantimes[cond,] <- apply(abs(EEGtimecourse[,cond,]),2,mean,na.rm=TRUE)
EEGSEtimes[cond,] <- apply(abs(EEGtimecourse[,cond,]),2,sd,na.rm=TRUE)/sqrt(nsubjs)
}

EEGmaskindices <- colMeans(20*log10(abs(EEGtimecourse[,3,])/abs(EEGtimecourse[,2,])))
EEGmaskSE <- apply(20*log10(abs(EEGtimecourse[,3,])/abs(EEGtimecourse[,2,])),2,sd)/sqrt(nsubjs)

times <- (-999:12000)/1000


postscript('pilotIM.ps', horizontal = FALSE, onefile = FALSE, paper = "special", height = 5, width = 5)
...
mtext(text = c(expression(sqrt(2)/2),'1',expression(sqrt(2)),'2'), side = 2, at=ticklocsy, line=0.2, las=1)
title(xlab="Time (s)", col.lab=rgb(0,0,0), line=1.2, cex.lab=1.5)    
title(ylab="12Hz response", col.lab=rgb(0,0,0), line=1.5, cex.lab=1.5)
...
if (plotsmoothed==0){
  ...
  polygon(times[c(1:13000,13000:1)],c(EEGmaskindices+EEGmaskSE,EEGmaskindices[13000:1]-EEGmaskSE[13000:1]),col=colpal[1],border=NA)

lines(times,EEGmaskindices,col=colpal[1],lwd=4)
}

if (plotsmoothed==1){

  spl5aU <- smooth.spline(times,EEGmaskindices+EEGmaskSE,df=20)
spl5aL <- smooth.spline(times,EEGmaskindices-EEGmaskSE,df=20)
polygon(times[c(1:13000,13000:1)],c(spl5aU$y,spl5aL$y[13000:1]),col=colpal[1],border=NA)

spl5a <- smooth.spline(times,EEGmaskindices,df=20)
lines(spl5a$x,spl5a$y,col=colpal[1],lwd=6)
}

text(9,5.5,'Pilot EEG data',cex=1.5)
dev.off()
...
EEGtimecourse12Hz <- array(0,dim=c(length(d),4,8000))
allwaves <- array(0,dim=c(length(d),4,9000))
allheads12 <- array(0,dim=c(length(d),4,64))
EEGtrialsincluded <- array(0,dim=c(length(d),4,64))
...
for (t in 1:(9000-binwidth)){
  spec <- fft(temp[t:(t+binwidth-1)])/binwidth
  fspec1[t] <- spec[findex]
}
    EEGtimecourse12Hz[s,cond,] <- fspec1

  for (ch in 1:64){
    temp <- fft(subjwaves[cond,1501:7500,ch])/6000
    allheads12[s,cond,ch] <- temp[1+(duration*(targetF+maskF))]
...
save(file='temp/EEGIM.RData',list=c('allheads12','allwaves','EEGtimecourse12Hz','electrodes','EEGtrialsincluded'))
}

if (processdata>0){
  
     if (!file.exists('temp/EEGIM.RData')){
      hid <- pmatch('EEGIM.RData',as.character(unlist(osffiles[,1])))
      osf_download(osffiles[hid,],'temp/',progress=TRUE)
 }
load('temp/EEGIM.RData')

nsubjs <- dim(EEGtimecourse12Hz)[1]    
mon1 <- 20*log10(abs(EEGtimecourse12Hz[,3,])/abs(EEGtimecourse12Hz[,1,]))
mon2 <- 20*log10(abs(EEGtimecourse12Hz[,3,])/abs(EEGtimecourse12Hz[,2,]))
meanmon <- (mon1 + mon2)/2
meanmon[which(is.infinite(meanmon))] <- 0
EEGmaskindicesMon <- colMeans(meanmon)
EEGmaskSEMon <- apply(meanmon,2,sd)/sqrt(nsubjs)

dich1 <- 20*log10(abs(EEGtimecourse12Hz[,4,])/abs(EEGtimecourse12Hz[,1,]))
dich2 <- 20*log10(abs(EEGtimecourse12Hz[,4,])/abs(EEGtimecourse12Hz[,2,]))
meandich <- (dich1 + dich2)/2
meandich[which(is.infinite(meandich))] <- 0
EEGmaskindicesDich <- colMeans(meandich)
EEGmaskSEDich <- apply(meandich,2,sd)/sqrt(nsubjs)

times <- (-999:7000)/1000

postscript('EEG_IM.ps', horizontal = FALSE, onefile = FALSE, paper = "special", height = 5, width = 5)
...
axis(1, at=ticklocsx, tck=0.01, lab=F, lwd=2)
axis(2, at=ticklocsy, tck=0.01, lab=F, lwd=2)
mtext(text = seq(-1,7,1), side = 1, at=seq(-1,7,1))
mtext(text = c(expression(sqrt(2)/2),'1',expression(sqrt(2)),'2'), side = 2, at=ticklocsy, line=0.2, las=1)
title(xlab="Time (s)", col.lab=rgb(0,0,0), line=1.2, cex.lab=1.5)    
title(ylab="12Hz response", col.lab=rgb(0,0,0), line=1.5, cex.lab=1.5)
...
if (plotsmoothed==0){
  ...
  polygon(times[c(1:8000,8000:1)],c(EEGmaskindicesMon+EEGmaskSEMon,EEGmaskindicesMon[8000:1]-EEGmaskSEMon[8000:1]),col=colpal[2],border=NA)
  polygon(times[c(1:8000,8000:1)],c(EEGmaskindicesDich+EEGmaskSEDich,EEGmaskindicesDich[8000:1]-EEGmaskSEDich[8000:1]),col=colpal[3],border=NA)

lines(times,EEGmaskindicesMon,col=colpal[2],lwd=4)
lines(times,EEGmaskindicesDich,col=colpal[3],lwd=4)
}

if (plotsmoothed==1){

spl5aU <- smooth.spline(times,EEGmaskindicesMon+EEGmaskSEMon,df=20)
spl5aL <- smooth.spline(times,EEGmaskindicesMon-EEGmaskSEMon,df=20)
polygon(times[c(1:8000,8000:1)],c(spl5aU$y,spl5aL$y[8000:1]),col=colpal[2],border=NA)

spl5aU <- smooth.spline(times,EEGmaskindicesDich+EEGmaskSEDich,df=20)
spl5aL <- smooth.spline(times,EEGmaskindicesDich-EEGmaskSEDich,df=20)
polygon(times[c(1:8000,8000:1)],c(spl5aU$y,spl5aL$y[8000:1]),col=colpal[3],border=NA)


spl5a <- smooth.spline(times,EEGmaskindicesMon,df=20)
lines(spl5a$x,spl5a$y,col=colpal[2],lwd=6)

spl5a <- smooth.spline(times,EEGmaskindicesDich,df=20)
lines(spl5a$x,spl5a$y,col=colpal[3],lwd=6)

}
text(5.5,5.5,'EEG data',cex=1.5)

legend(1.5,-0.5,c('Monocular','Dichoptic'),lwd=4,col=colpal[c(2,3)],bty='n')
...
d <- dir(path = 'temp/MEGdata', pattern = '*.RData', full.names=TRUE)
...
MEGtimecourse12Hz <- array(0,dim=c(length(d),4,8008))
MEGsubjwaves <- array(0,dim=c(length(d),4,9009))
allspec <- array(0,dim=c(length(d),4,(duration*20)))
MEGtrialsincluded <- array(0,dim=c(length(d),4))
...
for (s in 1:length(d)){
  ...
  for (cond in 1:4){
     ...
     temp <- alltrials[cond,1:trialcounter[1,cond],]
          ...
          for (t in 1:trialcounter[1,cond]){
            spec <- fft(temp[t,1501:7506])/6006
            spec12[t] <- spec[(targetF+maskF)*duration+1]
          }
        spec12[which(is.na(spec12))] <- 0

      if (length(which(abs(spec12)>0))>2){  
      tempxy <- data.frame(Re(spec12),Im(spec12))
      D12 <- sqrt(mahalanobis(tempxy,colMeans(tempxy),cov(tempxy)))
      i <- which(D12<3)
      MEGtrialsincluded[s,cond] <- length(i)
      temp <- colMeans(temp[i,])
      MEGsubjwaves[s,cond,] <- temp
      ...
      allspec[s,cond,] <- spec[1+(1:(duration*20))]
        ...
        MEGtimecourse12Hz[s,cond,] <- fspec1
...
save(file='temp/MEGIM.RData',list=c('MEGsubjwaves','MEGtimecourse12Hz','allspec','MEGtrialsincluded'))

}

if (processdata>0){
  
       if (!file.exists('temp/MEGIM.RData')){
      hid <- pmatch('MEGIM.RData',as.character(unlist(osffiles[,1])))
      osf_download(osffiles[hid,],'temp/',progress=TRUE)
 }
load('temp/MEGIM.RData')

nsubjs <- dim(MEGtimecourse12Hz)[1]    
mon1 <- 20*log10(abs(MEGtimecourse12Hz[,3,])/abs(MEGtimecourse12Hz[,1,]))
mon2 <- 20*log10(abs(MEGtimecourse12Hz[,3,])/abs(MEGtimecourse12Hz[,2,]))
meanmon <- (mon1 + mon2)/2
meanmon[which(is.infinite(meanmon))] <- 0
MEGmaskindicesMon <- colMeans(meanmon)
MEGmaskSEMon <- apply(meanmon,2,sd)/sqrt(nsubjs)

dich1 <- 20*log10(abs(MEGtimecourse12Hz[,4,])/abs(MEGtimecourse12Hz[,1,]))
dich2 <- 20*log10(abs(MEGtimecourse12Hz[,4,])/abs(MEGtimecourse12Hz[,2,]))
meandich <- (dich1 + dich2)/2
meandich[which(is.infinite(meandich))] <- 0
MEGmaskindicesDich <- colMeans(meandich)
MEGmaskSEDich <- apply(meandich,2,sd)/sqrt(nsubjs)

times <- (-999:7008)/1000

postscript('MEG_IM.ps', horizontal = FALSE, onefile = FALSE, paper = "special", height = 5, width = 5)
...
if (plotsmoothed==0){
  ...
  polygon(times[c(1:8008,8008:1)],c(MEGmaskindicesMon+MEGmaskSEMon,MEGmaskindicesMon[8008:1]-MEGmaskSEMon[8008:1]),col=colpal[2],border=NA)
  polygon(times[c(1:8008,8008:1)],c(MEGmaskindicesDich+MEGmaskSEDich,MEGmaskindicesDich[8008:1]-MEGmaskSEDich[8008:1]),col=colpal[3],border=NA)

lines(times,MEGmaskindicesMon,col=colpal[2],lwd=4)
lines(times,MEGmaskindicesDich,col=colpal[3],lwd=4)
}

if (plotsmoothed==1){

spl5aU <- smooth.spline(times,MEGmaskindicesMon+MEGmaskSEMon,df=20)
spl5aL <- smooth.spline(times,MEGmaskindicesMon-MEGmaskSEMon,df=20)
polygon(times[c(1:8008,8008:1)],c(spl5aU$y,spl5aL$y[8008:1]),col=colpal[2],border=NA)

spl5aU <- smooth.spline(times,MEGmaskindicesDich+MEGmaskSEDich,df=20)
spl5aL <- smooth.spline(times,MEGmaskindicesDich-MEGmaskSEDich,df=20)
polygon(times[c(1:8008,8008:1)],c(spl5aU$y,spl5aL$y[8008:1]),col=colpal[3],border=NA)


spl5a <- smooth.spline(times,MEGmaskindicesMon,df=20)
lines(spl5a$x,spl5a$y,col=colpal[2],lwd=6)

spl5a <- smooth.spline(times,MEGmaskindicesDich,df=20)
lines(spl5a$x,spl5a$y,col=colpal[3],lwd=6)

}
text(5.5,5.5,'MEG data',cex=1.5)
...
dev.off()
  ...
  PostScriptTrace(paste('pilotIM.ps',sep=''))
  e1 <- readPicture('pilotIM.ps.xml')
  ...
  PostScriptTrace(paste('EEG_IM.ps',sep=''))
  e2 <- readPicture('EEG_IM.ps.xml')
  for (n in 1:length(e2@paths)){
    temp <- class(e2@paths[n]$path)[1]
    if (pmatch(temp,"PictureFill",nomatch=0)){
      if (sum(col2rgb(e2@paths[n]$path@rgb))<765){e2@paths[n]$path@rgb <- addalpha(e2@paths[n]$path@rgb,alpha=0.2)}}}
  
  PostScriptTrace(paste('MEG_IM.ps',sep=''))
  e3 <- readPicture('MEG_IM.ps.xml')
  for (n in 1:length(e3@paths)){
    temp <- class(e3@paths[n]$path)[1]
    if (pmatch(temp,"PictureFill",nomatch=0)){
      if (sum(col2rgb(e3@paths[n]$path@rgb))<765){e3@paths[n]$path@rgb <- addalpha(e3@paths[n]$path@rgb,alpha=0.2)}}}
      
if (fixedphase==0){pdf('Figures/IMfigure.pdf', bg="transparent", height = 5, width = 18)}

  par(mar=c(0.1,0.1,0.1,0.1))
  plot(x=NULL,y=NULL,axes=FALSE, ann=FALSE, xlim=c(0,1), ylim=c(0,1))
  
  grid.picture(e1,x=1/6,y=1/2,width=0.33,height=1)
  grid.picture(e2,x=3/6,y=1/2,width=0.33,height=1)
  grid.picture(e3,x=5/6,y=1/2,width=0.33,height=1)
  
  text(-0.04,0.97,'(a)',pos=4,cex=3)
  text(0.32,0.97,'(b)',pos=4,cex=3)
  text(0.68,0.97,'(c)',pos=4,cex=3)

  dev.off()
  
  file.remove(c('pilotIM.ps','EEG_IM.ps','MEG_IM.ps'))
  file.remove(c('pilotIM.ps.xml','EEG_IM.ps.xml','MEG_IM.ps.xml'))
...
```{r runAQanalysis, include=FALSE, results='hide'}

# AQ analysis and figure plotting
...
if (processdata>0){
  ...
  PilotmaskindexL <- colMeans(monratios,na.rm=TRUE)
  PilotmaskindexSEL <- apply(monratios,2,sd,na.rm=TRUE)/sqrt(sum(includedboth))
  ...
  monratios4 <- abs(EEGtimecourse[which(includedboth==1),3,1,])/abs(EEGtimecourse[which(includedboth==1),3,3,])
  monratios <- (20*log10(monratios1) + 20*log10(monratios2) + 20*log10(monratios3) + 20*log10(monratios4))/4
  PilotmaskindexH <- colMeans(monratios,na.rm=TRUE)
  PilotmaskindexSEH <- apply(monratios,2,sd,na.rm=TRUE)/sqrt(sum(includedboth))
  ...
  Pilotmaskindex5L[1,] <- colMeans(20*log10(monratios),na.rm=TRUE)
  PilotmaskindexSE5L[1,] <- apply(20*log10(monratios),2,sd,na.rm=TRUE)/sqrt(sum(includedboth))
  
  Pilotmaskindex5H <- matrix(0,nrow=2,ncol=13000)
  PilotmaskindexSE5H <- matrix(0,nrow=2,ncol=13000)
  includedboth <- isincluded[2,]*isincluded[3,]*ishighAQp
  monratios <- abs(EEGtimecourse[which(includedboth==1),2,2,])/abs(EEGtimecourse[which(includedboth==1),2,3,])
  Pilotmaskindex5H[1,] <- colMeans(20*log10(monratios),na.rm=TRUE)
  PilotmaskindexSE5H[1,] <- apply(20*log10(monratios),2,sd,na.rm=TRUE)/sqrt(sum(includedboth))
  ...
  Pilotmaskindex7L[1,] <- colMeans(20*log10(monratios),na.rm=TRUE)
  PilotmaskindexSE7L[1,] <- apply(20*log10(monratios),2,sd,na.rm=TRUE)/sqrt(sum(includedboth))
  ...
  monratios <- abs(EEGtimecourse[which(includedboth==1),1,1,])/abs(EEGtimecourse[which(includedboth==1),1,3,])
  Pilotmaskindex7H[1,] <- colMeans(20*log10(monratios),na.rm=TRUE)
  PilotmaskindexSE7H[1,] <- apply(20*log10(monratios),2,sd,na.rm=TRUE)/sqrt(sum(includedboth))
  ...
  EEGmaskindexL[1,] <- colMeans(monratios,na.rm=TRUE)
  EEGmaskindexSEL[1,] <- apply(monratios,2,sd,na.rm=TRUE)/sqrt(sum(includedboth))
  ...
  EEGmaskindexL[2,] <- colMeans(dichratios,na.rm=TRUE)
  EEGmaskindexSEL[2,] <- apply(dichratios,2,sd,na.rm=TRUE)/sqrt(sum(includedboth))
  ...
  monratios2 <- abs(EEGtimecourse7Hz[which(includedboth==1),2,])/abs(EEGtimecourse7Hz[which(includedboth==1),3,])
  monratios <- (20*log10(monratios1) + 20*log10(monratios2))/2
  EEGmaskindexH[1,] <- colMeans(monratios,na.rm=TRUE)
  EEGmaskindexSEH[1,] <- apply(monratios,2,sd,na.rm=TRUE)/sqrt(sum(includedboth))
  includedboth <- isincluded[1,]*isincluded[4,]*ishighAQ
  dichratios1 <- abs(EEGtimecourse5Hz[which(includedboth==1),1,])/abs(EEGtimecourse5Hz[which(includedboth==1),4,])
  dichratios2 <- abs(EEGtimecourse7Hz[which(includedboth==1),2,])/abs(EEGtimecourse7Hz[which(includedboth==1),4,])
  dichratios <- (20*log10(dichratios1) + 20*log10(dichratios2))/2
  EEGmaskindexH[2,] <- colMeans(dichratios,na.rm=TRUE)
  EEGmaskindexSEH[2,] <- apply(dichratios,2,sd,na.rm=TRUE)/sqrt(sum(includedboth))
  ...
  includedboth <- isincluded[1,]*isincluded[3,]*islowAQ
  monratios <- abs(EEGtimecourse5Hz[which(includedboth==1),1,])/abs(EEGtimecourse5Hz[which(includedboth==1),3,])
  EEGmaskindex5L[1,] <- colMeans(20*log10(monratios),na.rm=TRUE)
  EEGmaskindexSE5L[1,] <- apply(20*log10(monratios),2,sd,na.rm=TRUE)/sqrt(sum(includedboth))
  includedboth <- isincluded[1,]*isincluded[4,]*islowAQ
  dichratios <- abs(EEGtimecourse5Hz[which(includedboth==1),1,])/abs(EEGtimecourse5Hz[which(includedboth==1),4,])
  EEGmaskindex5L[2,] <- colMeans(20*log10(dichratios),na.rm=TRUE)
  EEGmaskindexSE5L[2,] <- apply(20*log10(dichratios),2,sd,na.rm=TRUE)/sqrt(sum(includedboth))
  ...
  CgroupM[which(is.na(CgroupM))] <- 0
  AgroupM[which(is.na(AgroupM))] <- 0
  CgroupD[which(is.na(CgroupD))] <- 0
  AgroupD[which(is.na(AgroupD))] <- 0
  ...
  for (masktype in 1:2){
    ...
    MEGmaskindexC[which(is.na(MEGmaskindexC))] <- 0
    MEGmaskindexA[which(is.na(MEGmaskindexA))] <- 0
    MEGmaskindexSEC[which(is.na(MEGmaskindexSEC))] <- 0
    MEGmaskindexSEA[which(is.na(MEGmaskindexSEA))] <- 0
...
if (fixedphase==0){pdf('Figures/AQfigure.pdf', bg="transparent", height = 10, width = 12)}
if (fixedphase==1){pdf('Figures/AQfigureFP.pdf', bg="transparent", height = 10, width = 12)}
...
Suppressive interactions between neurons are ubiquitous in the nervous system, with normalization considered a canonical neuronal computation [@Carandini2011]. One consequence of normalization is that neurons tuned to different stimulus features modulate each others' firing, usually via a process of divisive suppression [@Heeger1992]. For decades the strength of suppression was treated as fixed, due to the observation that adapting to one stimulus does not decrease its suppressive potency [@Foley1997; @Freeman2002]. This orthodoxy has been challenged by a series of innovative studies proposing that normalization can be 'reweighted' by recent history [@Westrick2016; @Aschner2018; @Yiltiz2020]. When pairs of stimuli are repeatedly presented together, their neural representations appear to suppress each other more strongly. Far from being fixed, normalization may therefore be a dynamic process that is continuously updated by the sensory environment. Our objectives were to measure the timecourse of changes in suppression non-invasively in the human brain, compare them across distinct anatomical pathways, and determine whether they differ as a function of autistic traits.

Plastic changes within the visual system occur over multiple timescales [see @Webster2015 for a recent review]. Cortical forms of adaptation to cues such as stimulus contrast [@Blakemore1969], orientation [@Gibson1937] and motion [@Mather2008] can be observed within a few seconds, but also build up over durations on the order of minutes [@Greenlee1991]. Other types of adaptation have been identified where changes occur over longer time periods, such as several hours [@Kwon2009] or days [@Haak2014]. Previous normalization reweighting studies involved adapting sequences of around 40 -- 60s [@Aschner2018; @Yiltiz2020], but in principle reweighting might occur faster than this, consistent with other types of contrast adaptation.

Multiple suppressive pathways have been identified in the visual system, including between stimuli differing in orientation [@Heeger1992; @Foley1994], eye-of-origin [@Legge1979; @Sengpiel1994; @Dougherty2019] and spatial position [@Petrov2005; @Cannon1991]. At present there is evidence of normalization reweighting between stimuli with orthogonal orientations [@Aschner2018], and adjacent spatial positions [@Yiltiz2020]. We anticipated that interocular suppression should also be subject to reweighting, but that there might be differences in the dynamics across suppressive pathways [e.g. @Meese2009; @Sengpiel2005; @Li2005]. Comparing monocular and dichoptic suppression permits any contribution of early pre-cortical factors to be isolated. This is because interocular suppression is generally thought to impact in primary visual cortex [though see @Dougherty2019], and bypasses any retinal and subcortical stages of processing that contribute to monocular suppression [@Li2005].

Atypical sensory experience, including hypersensitivity to loud sounds, bright lights and strong odours or flavours, is widely reported by individuals on the autism spectrum [@MacLennan2022; @Simmons2009], but the causal mechanisms remain unclear. Fundamental measures of sensitivity including visual acuity [@Tavassoli2011], contrast sensitivity [@Koh2010], and audiometric performance [@Rosenhall1999] are not consistently different from neurotypical controls. Theoretical accounts of sensory differences in autism have proposed that the balance of inhibition and excitation may be disrupted [@Rubenstein2003; @Rosenberg2015], yet the evidence is currently inconclusive [e.g. @Schallmo2020; @VandeCruys2018; @Sandhu2020]. Our recent work identified an autism-related difference using steady-state EEG [@Vilidaite2018], in which nonlinear (second harmonic) responses were weaker, implicating atypical suppression in autism.
...
## Preregistration, data and code accessibility
...
if (fixedphase==0){knitr::include_graphics('Figures/AQfigure.pdf')}
if (fixedphase==1){knitr::include_graphics('Figures/AQfigureFP.pdf')}
