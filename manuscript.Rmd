...
packagelist <- c('knitr','remotes','tictoc','R.matlab','bookdown','grImport','tiff','pals','ez','gtools') # list of CRAN packages
...
dev.off()
  ...
  pdf('temp/figures/Fig1.pdf', bg="transparent", height = 6, width = 16)
...
knitr::include_graphics('temp/figures/Fig1.pdf')
...
save(file='temp/EEGsummary.RData',list=c('allheads5','allheads7','allspec','allwaves','EEGtimecourse5Hz','EEGtimecourse7Hz','electrodes','allAQ','allSPQ'))
 }
...
# pdf("temp/figures/Fig2.pdf", bg="transparent", height = 10, width = 12)
# 
# par(mfrow=c(3,4))
...
# 
  ...
  pdf('temp/figures/Fig2.pdf', bg="transparent", height = 6, width = 15)
  par(mar=c(0.1,0.1,0.1,0.1))
  plot(x=NULL,y=NULL,axes=FALSE, ann=FALSE, xlim=c(0,1), ylim=c(0,1)) 
...
if (processdata>1){
  ...
  d <- dir(path = 'temp/wang/', pattern = '*alltrialsFullV1.mat', full.names=TRUE)
  d2 <- dir(path = 'temp/wang/', pattern = '*alltrialsFullV1.mat', full.names=FALSE)
  ...
  for (s in 1:length(d)){
    ...
    data <- readMat(d[s])
      ...
      for (cond in 1:4){
        temp <- colMeans(data$alltrials[cond,1:data$trialcounter[1,cond],],na.rm=TRUE)
        nainds <- which(is.na(temp))   # find occasional NaN values in the waveform
        temp[nainds] <- 0   # replace with zeros
        # allwaves[s,cond,] <- temp - mean(temp)
        spec <- fft(temp[1501:7500])/6000
  ...
  save(file='temp/MEGsummaryV1.RData',list=c('allspec','MEGtimecourse5Hz','MEGtimecourse7Hz','allRnos'))

}

if (processdata>0){
  load('temp/MEGsummaryV1.RData')
...
if (nbootstraps>0){clustoutD7 <- clustercorrect(earlydataD,latedataD,testtype=4,paired=TRUE)}
  ...
  e1 <- readTIFF('temp/figures/groupbrainbothSNR.tif')
  e2 <- readTIFF('temp/figures/LH.tif')
  e3 <- readTIFF('temp/figures/RH.tif')
...
pilotAQ <- read.csv('temp/AQscores.csv',header=FALSE)
pilotAQ <- pilotAQ[-75,2]

load("temp/questionnairedataMSci.RData")
MEGAQ <- qdata$AQ[1:19]
MEGgroup <- qdata$Diagnosis[1:19]

MEG_A <- MEGAQ[which(MEGgroup=="Yes")]
MEG_C <- MEGAQ[which(MEGgroup=="No")]
t <- t.test(MEG_A,MEG_C,paired=FALSE)

t_age <- t.test(qdata$Age[which(qdata$Diagnosis=='Yes')],qdata$Age[which(qdata$Diagnosis=='No')],paired=FALSE,var.equal=TRUE)

corMEG <- cor.test(qdata$AQ[1:19],qdata$SPQ[1:19])
...
if (processdata>0){
  ...
  load('temp/MEGsummaryV1.RData')
  MEGgroup <- qdata$Diagnosis
  MEGID <- qdata$ID
  dataorder <- match(MEGID,allRnos)
  # dataorder tells us the indices into the MEG data for each AQ score
  ...
  CONindices <- dataorder[which(MEGgroup=='No')]
  ASCindices <- dataorder[which(MEGgroup=='Yes')]
  isCON <- (1:20)*0
  isASC <- (1:20)*0
  isCON[CONindices] <- 1
  isASC[ASCindices] <- 1
  ...
  pdf('temp/figures/Fig4.pdf', bg="transparent", height = 10, width = 12)
  par(mar=c(0.1,0.1,0.1,0.1))
  plot(x=NULL,y=NULL,axes=FALSE, ann=FALSE, xlim=c(0,1), ylim=c(0,1))
